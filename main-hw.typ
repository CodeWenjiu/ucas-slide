...
*解答.* 由 $L$-Lipschitz 连续性可得下降引理

$ L(bold(w)^(k+1)) <= L(bold(w)^(k)) - eta (1 - eta L/2) norm(gradient L(bold(w)^(k)))_2^2, $

因此当 $0 < eta <= 1/L$ 时目标函数单调不增；进一步结合凸性可得收敛速率
$O(1\/k)$。详细推导见附录。
